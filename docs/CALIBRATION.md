...
1. Start it: `marionette calibrate pair -c config.json`. The reference is the
   first node in the config; with more than two nodes, calibrate each target
   against the reference one at a time.
...
Verify by running `marionette run` and watching the fused output stay solid as
you turn in place: a bad pair calibration shows up as limbs splitting into
doubles when you rotate between views.
...
1. `marionette calibrate body -c config.json`
...
2. `marionette calibrate playspace -c config.json`
...
5. The anchor is written to `calibration.json` under `world_anchor` and is
   applied by the OpenVR bridge on the next `marionette run`.
